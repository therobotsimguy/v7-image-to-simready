...
def run_test(asset_path, num_steps=500, scale=None):
    ...
    # T3: Contact test — count collision contacts at rest
    print(f"\n  [T3] Contact analysis...")
    sim.reset()
    for _ in range(120):  # 1 second settle
        sim.step()

    # Use PhysX contact reporting
    try:
        physx_interface = sim._physics_sim_view
        if physx_interface is not None:
            record("T3", "PhysX contact reporting", "PASS", "physics view available")
        else:
            record("T3", "PhysX contact reporting", "WARN", "no physics view — contact data unavailable")
    except Exception as e:
        record("T3", "PhysX contact reporting", "WARN", f"contact query failed: {e}")

    # T4: Gravity test — do dynamic bodies fall correctly?
    print(f"\n  [T4] Gravity response...")
    sim.reset()
    ...
    if fell_through > 0:
        record("T4", "Gravity response", "FAIL", f"{fell_through} bodies fell through ground")
    else:
        record("T4", "Gravity response", "PASS", "all dynamic bodies above ground")
